...
export async function register(req, res) {
    ...
    try{
        ...
        if(existingUser){
            return res.status(400).json({message:"Email already in use. Use another."});
        ...
        res.status(500).json({message: "Registration Error. Refresh the page and try again. If issues persist, reach out to developers."});
